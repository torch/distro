...
call %1 || (echo failed to run torchenv.cmd, can not setup torch environment && goto :Fail)

for /f %%i in ('where luajit.cmd') do set LUA=%%i
if exist "%LUA%" (
  set TORCH_USE_LUAJIT=1
) else (
  for /f %%i in ('where lua.cmd') do set LUA=%%i
...
:HELP
echo Usage: test.bat path_to_torchenv.cmd
